...
import { Copy, RefreshCw, CircleCheck, CircleAlert, Pencil, Image as ImageIcon, User, Sparkles, Download, LayoutTemplate, Video, Lightbulb, Cpu } from 'lucide-react';
...
const App = () => {
  ...
  const [selectedTextEngine, setSelectedTextEngine] = useState('chatgpt'); 
  ...
  const generateIdea = async (field) => {
    ...
    let promptText = "";
    if (field === 'yt_topic') promptText = "幫我發想一個極具點擊率 (Clickbait)、充滿數據或心理學乾貨的德州撲克 YouTube 影片主題。只輸出『一句話』，不要引號。";
    else if (field === 'custom') promptText = "請給我一個適合在 Threads 上引起德州撲克玩家激烈討論的爭議性話題或進階心法。只輸出『一句話』，不要引號。";
    else if (field === 'quiz') promptText = "設計一個德州撲克實戰中會遇到的困難情境。只要丟出情境，不用給答案，只輸出『一句話』，不要引號。";
  ...
  const callAI = async (prompt, systemInstruction, type) => {
    ...
    try {
      const text = await fetchAIContent(prompt, systemInstruction);
      if (text) setResults(prev => ({ ...prev, [key]: text.trim() }));
    } catch (err) {
  ...
  const generateText = (platform) => {
    ...
    if (activeTab === 'schedule') {
      ...
      prompt = platform === 'ig' ? `${schedule.prompt}\n\n【擴寫為 Instagram 長文文案。包含豐富細節、良好排版、Emoji，以及 10-15 個 IG 撲克熱門 Hashtags。】` : schedule.prompt;
    } else if (activeTab === 'custom') {
      if (!inputs.custom.trim()) return setErrors(prev => ({ ...prev, [currentKey]: "請輸入主題" }));
      prompt = platform === 'ig' ? `針對主題「${inputs.custom}」寫一篇IG長文。包含排版及10-15個Hashtags。` : `針對主題「${inputs.custom}」寫一篇 Threads 短文，文末加上 —— @FlopScience`;
    } else if (activeTab === 'hh') {
      if (!inputs.hh.trim()) return setErrors(prev => ({ ...prev, [currentKey]: "請貼上牌譜" }));
      prompt = `將這段牌局紀錄轉譯成沉浸式故事：「${inputs.hh}」。保留數據，第一人稱，文末加上 —— @FlopScience。`;
    } else if (activeTab === 'quiz') {
      if (!inputs.quiz.trim()) return setErrors(prev => ({ ...prev, [currentKey]: "請輸入測驗主題" }));
      prompt = `根據主題：「${inputs.quiz}」，設計一個 Threads 德撲情境選擇題。提供 A,B,C 選項。不要公佈答案。`;
    } else if (activeTab === 'youtube') {
      if (!inputs.yt_topic.trim()) return setErrors(prev => ({ ...prev, [currentKey]: "請輸入影片主題！" }));
      let lengthInstructions = inputs.yt_length.includes("60秒") ? "Shorts 節奏極快，前 3 秒抓住眼球。" : "中長度科普或深度解析。包含 3 個吸睛標題、時間軸與畫面分鏡、逐字稿旁白。";
      prompt = `以「${inputs.yt_topic}」為主題寫一份 YouTube 影片腳本。長度：「${inputs.yt_length}」。\n${lengthInstructions}\n語氣：「${tone}」。`;
    } else if (activeTab === 'reply') {
      if (!inputs.reply.trim()) return setErrors(prev => ({ ...prev, [currentKey]: "請貼上留言" }));
      prompt = `網友留言：「${inputs.reply}」。請用「${tone}」語氣寫一段 50 字內的回覆。`;
    }

    callAI(prompt, getSystemInstruction(), platform || 'default');
  };

  const generateImagePrompt = async () => {
    if (activeTab === 'youtube') return generateYTImages();
    const key = currentKey;
    ...
    try {
      const extractPrompt = `Create a 1-sentence English image prompt from this text: "${currentText.substring(0, 500)}".
Must enforce this aesthetic: "${inputs.image_style}".
Target AI: ${selectedImageModel}. 4k resolution, highly detailed, NO TEXT, NO LETTERS.`;
      
      const text = await fetchAIContent(extractPrompt, "You are an expert prompt engineer.");
  ...
  const generateYTImages = async () => {
    ...
    try {
      let requiredImages = inputs.yt_image_count === 'auto' ? 4 : parseInt(inputs.yt_image_count, 10);
      const extractPrompt = `請閱讀以下腳本並萃取需要 B-roll 的段落：\n"${currentText.substring(0, 1000)}"\n
請嚴格輸出一個包含 "scenes" 屬性的 JSON 物件，其值為一個陣列。陣列內必須剛好包含 ${requiredImages} 個物件。
每個物件包含：
"description": 繁體中文，對應腳本段落說明。
"prompt": 英文，AI 繪圖指令。風格：「${inputs.image_style}」。NO TEXT。`;

      const text = await fetchAIContent(extractPrompt, "You only output JSON.", true);
      const parsed = JSON.parse(text);
      if (parsed.scenes && Array.isArray(parsed.scenes)) {
         setYtImages(parsed.scenes);
      } else {
         throw new Error("JSON 格式不符");
      }
    } catch (err) {
      setImageErrors(prev => ({ ...prev, [key]: err.message || "配圖 Prompt 生成失敗" }));
    }
          ...
          <div className="p-6 flex flex-col md:flex-row gap-6">
            ...
            <div className="md:w-5/12 flex flex-col gap-4">
              ...
              {activeTab === 'custom' && (
                <div className="flex flex-col gap-2">
                  ...
                  <textarea className="w-full bg-slate-950 border border-slate-700 p-4 rounded-xl min-h-[120px] resize-y text-sm focus:border-emerald-500 outline-none" value={inputs.custom} onChange={(e) => handleInputChange('custom', e.target.value)} />
                </div>
              )}

              {activeTab === 'hh' && <textarea className="w-full bg-slate-950 border border-slate-700 p-4 rounded-xl min-h-[120px] font-mono text-sm focus:border-emerald-500 outline-none" placeholder="貼上生硬牌譜數據..." value={inputs.hh} onChange={(e) => handleInputChange('hh', e.target.value)} />}

              {activeTab === 'quiz' && (
                 <div className="flex flex-col gap-2">
                  <div className="flex justify-between items-center px-1">
                    <span className="text-xs text-slate-400 font-bold">測驗情境</span>
                    <button onClick={() => generateIdea('quiz')} disabled={ideaLoading['quiz']} className="text-xs text-emerald-400 hover:text-emerald-300 flex items-center gap-1">
                      {ideaLoading['quiz'] ? <RefreshCw size={12} className="animate-spin" /> : <Lightbulb size={12} />} 隨機生成測驗情境
                    </button>
                  </div>
                  <textarea className="w-full bg-slate-950 border border-slate-700 p-4 rounded-xl min-h-[120px] text-sm focus:border-emerald-500 outline-none" value={inputs.quiz} onChange={(e) => handleInputChange('quiz', e.target.value)} />
                </div>
              )}
                ...
                <div className="flex flex-col gap-3">
                  <div className="flex flex-col gap-2">
                    <div className="flex justify-between items-center px-1">
                      ...
                      <button onClick={() => generateIdea('yt_topic')} disabled={ideaLoading['yt_topic']} className="text-xs text-red-400 flex items-center gap-1 bg-red-950/30 px-2 py-1 rounded border border-red-900/50">
                        {ideaLoading['yt_topic'] ? <RefreshCw size={12} className="animate-spin" /> : <Lightbulb size={12} />} 🎲 隨機爆款話題
                      </button>
                    </div>
                    <textarea className="w-full bg-slate-950 border border-slate-700 p-4 rounded-xl min-h-[100px] text-sm focus:border-red-500 outline-none" value={inputs.yt_topic} onChange={(e) => handleInputChange('yt_topic', e.target.value)} />
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div className="bg-slate-950 p-3 rounded-xl border border-slate-800">
                      <label className="block text-xs text-slate-400 font-bold mb-2">⏱️ 影片長度預估</label>
                      <select className="w-full bg-slate-900 border border-slate-700 text-slate-200 rounded-lg text-sm py-2 px-2 outline-none focus:border-red-500" value={inputs.yt_length} onChange={(e) => handleInputChange('yt_length', e.target.value)}>
                    ...
                    <div className="bg-slate-950 p-3 rounded-xl border border-slate-800">
                      <label className="block text-xs text-slate-400 font-bold mb-2">🖼️ 自訂分鏡數量</label>
                      <select className="w-full bg-slate-900 border border-slate-700 text-slate-200 rounded-lg text-sm py-2 px-2 outline-none focus:border-red-500" value={inputs.yt_image_count} onChange={(e) => handleInputChange('yt_image_count', e.target.value)}>
                        <option value="auto">自動 (依片長決定)</option>
                        {[1,2,3,4,5,6].map(n => <option key={n} value={n}>{n} 組</option>)}
                      </select>
                    </div>
                  </div>
                </div>
              ...
              {activeTab === 'reply' && <textarea className="w-full bg-slate-950 border border-slate-700 p-4 rounded-xl min-h-[120px] text-sm focus:border-emerald-500 outline-none" placeholder="貼上網友留言..." value={inputs.reply} onChange={(e) => handleInputChange('reply', e.target.value)} />}
              ...
              <div className="flex flex-col gap-3 mt-2">
                ...
                <div className="bg-slate-950 p-3 rounded-xl border border-slate-800">
                  <label className="block text-xs text-slate-500 font-bold mb-2 uppercase text-blue-400">目標 AI 繪圖工具 (產出指令格式)</label>
                  <select className="w-full bg-slate-900 border border-slate-700 text-slate-200 rounded-lg text-sm py-2 px-2 outline-none focus:border-blue-500" value={selectedImageModel} onChange={(e) => setSelectedImageModel(e.target.value)}>
            ...
            <div className="md:w-7/12 flex flex-col">
              <div className="flex items-center justify-between mb-4">
                ...
                {results[currentKey] && (
                  <button onClick={() => generateImagePrompt()} disabled={(activeTab === 'youtube' ? ytImageLoading : imageLoading[currentKey])} className="text-xs bg-slate-800 text-slate-300 border border-slate-700 px-3 py-1.5 rounded-lg flex items-center gap-1 hover:bg-slate-700 transition-colors">
                    ...
                    {activeTab === 'youtube' ? `萃取配圖 Prompt` : '寫入 AI 繪圖 Prompt'}
                  </button>
                )}
              </div>
              ...
              <div className="flex-1 bg-slate-950 border border-slate-700 rounded-xl p-5 relative flex flex-col min-h-[300px]">
                {loading[currentKey] ? (
                  <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-950/80 z-10 rounded-xl backdrop-blur-sm">
                    ...
                    <span className="text-sm text-slate-400 animate-pulse">正在透過後端安全通道構思內容...</span>
                  </div>
                ) : errors[currentKey] ? (
                  ...
                  <>
                    ...
                    <div className="flex justify-end gap-3 mt-4 pt-4 border-t border-slate-800">
                      ...
                      <button onClick={() => copyText(results[currentKey], currentKey)} className={`text-sm ${activeTab === 'youtube' ? 'bg-red-600 hover:bg-red-500' : 'bg-emerald-600 hover:bg-emerald-500'} px-5 py-2 rounded-lg flex items-center gap-1.5 text-white font-bold`}>
              ...
              {activeTab === 'youtube' ? (
                 (ytImageLoading || ytImages.length > 0 || imageErrors['youtube']) && (
                  <div className="mt-4 bg-slate-950 border border-slate-800 rounded-xl p-4 flex flex-col items-center justify-center relative min-h-[140px]">
                    {ytImageLoading ? (
                       <div className="flex flex-col items-center text-slate-500 animate-pulse"><Video className="w-8 h-8 mb-3 opacity-50" /><span>正在分析腳本並生成 Prompt...</span></div>
                    ) : imageErrors['youtube'] ? (
                      ...
                      <div className="w-full">
                        <p className="text-xs text-slate-400 mb-3 text-center border-b border-slate-800 pb-2">已為腳本萃取配圖 Prompt，請複製到 {selectedImageModel} 使用</p>
                          ...
                          {ytImages.map((img, idx) => (
                            <div key={idx} className="bg-slate-900 rounded-lg p-3 border border-slate-700">
                              ...
                              <div className="relative">
                                ...
                                <button onClick={() => copyText(img.prompt)} className="absolute top-2 right-2 p-1.5 bg-slate-800 text-slate-300 rounded hover:bg-slate-700"><Copy size={14}/></button>
                              </div>
                            </div>
                          ))}
                        </div>
                    ...
                    {imageLoading[currentKey] ? (
                      ...
                      <div className="w-full">
                        <div className="flex justify-between items-center mb-2">
                          <span className="text-sm font-bold text-emerald-400 flex items-center gap-1"><ImageIcon size={16}/> 生成專屬圖片 Prompt ({selectedImageModel})</span>
